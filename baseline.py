...
#lr training
def training(learningrate, batch):
    ...
    for epoch in range(iterations):
        ...
        #mini-batch loop
        for start in range(0, len(xtrainshuffled), batch):
            endindex = min(start + batch, len(xtrainshuffled)) #find ending index of current batch
            ...
            weight = weight - learningrate * gradient #w <- w - ndeltaW
        trainloss = crossentropyloss(xtrain, ytrainonehot, weight)
        valloss = crossentropyloss(xvalidation, yvalonehot, weight)
        ...
        validationlosshistory.append(valloss)
...
for i in learningrates:
    for j in batch:
        weight, history = training(i, j) #training with one learning rate and batch size
        ...
        hyperparameterresults.append((i, j, finalvalidationaccuracy, finalvalidationf1))
        print(f"learning rate: {i}. batch size: {j}. validation accuracy: {finalvalidationaccuracy:.4f}. validation macrof1: {finalvalidationf1:.4f}")
...
testloss = crossentropyloss(xtest, ytestonehot, bestweight)
...
print("learning rate - batch size - validation accuracy - validation macrof1")
for i, j, validationaccuracy, validationf1 in hyperparameterresults:
    print(f"{i:<13} - {j:<10} - {validationaccuracy:.4f} - {validationf1:.4f}")
...
plt.title("baseline logistic regression: training vs validation loss")
plt.xlabel("epochs")
plt.ylabel("cross entropy loss")
plt.legend()
plt.tight_layout()
plt.savefig("baselinelosscurve.png")
plt.show()
...
plt.title("baseline logistic regression: training vs validation accuracy")
plt.xlabel("epochs")
plt.ylabel("accuracy")
plt.legend()
plt.tight_layout()
plt.savefig("baselineaccuracycurve.png")
plt.show()
#validation macrof1
plt.figure()
plt.plot(epochs, besthistory["validationf1"], label = "validation macrof1")
plt.title("baseline logistic regression: validation macro F1")
plt.xlabel("epochs")
plt.ylabel("macrof1")
plt.legend()
plt.tight_layout()
plt.savefig("baselinemacrof1curve.png")
plt.show()
...
plt.colorbar()
plt.tight_layout()
plt.savefig("baselinetestconfusinonmatrix.png")
plt.show()
#save best model
np.savez("baselinebestmodel.npz", weight = bestweight, labels = np.array(gestures), learningrate = bestparameters[0], batch = bestparameters[1], testaccuracy = testaccuracy, testmacrof1 = testf1)